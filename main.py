...
def makeRectText(x, y, message, size):
    ...
    text_Title = FONT.render('{:,}'.format(int(message)), True, GRAY2)
...
while True:
    ...
    if int(seconds) % 300 == 0:
        ...
        if int(bp_prior_subs) > int(bp_subs) + 10000:
            bp_prior_subs = str(int(bp_prior_subs) - random.randint(2000, 4000))
